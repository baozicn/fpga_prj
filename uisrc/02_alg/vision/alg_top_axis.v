// alg_top_axis.v
module alg_top_axis #(
    parameter IMG_W=1920, IMG_H=1080, parameter SOBEL_T=80, parameter FDIFF_T=25
)(
    input wire clk, rst_n,
    input wire s_tvalid, input wire [7:0] s_tdata, input wire s_tuser, input wire s_tlast,
    input wire [1:0] sel,
    output wire m_tvalid, output wire [7:0] m_tdata, output wire m_tuser, output wire m_tlast
);
    ...
    axis_sobel3x3 #(.IMG_W(IMG_W),.IMG_H(IMG_H),.THRESH(SOBEL_T)) u_sobel(
        .clk(clk),.rst_n(rst_n),.s_tvalid(s_tvalid),.s_tdata(s_tdata),.s_tuser(s_tuser),.s_tlast(s_tlast),
    ...
    video_mux3_axis u_mux(.clk(clk),.rst_n(rst_n),.sel(sel),
